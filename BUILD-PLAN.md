...
| **FR-2** | **Record a source URL and retrieval timestamp for every registry field.**<br>*Why:* pricing and context claims get quoted back at us.<br>*Accept:* every populated field on any `model_version` row has a matching entry in `sources`. Applies to seeded rows too. | E1 |
