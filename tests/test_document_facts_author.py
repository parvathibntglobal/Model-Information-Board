...
def test_a_known_author_reaches_the_facts():
    conn = _Conn([("doc1", "reddit", date(2026, 8, 1), "auth_42", True, False, None, True)])
...
def test_a_null_author_stays_none_not_invented():
    ...
    conn = _Conn([("doc1", "blog", date(2026, 8, 1), None, True, False, None, True)])
...
def test_the_author_is_read_from_the_row_not_defaulted_away():
    ...
    conn = _Conn([
        ("doc1", "reddit", date(2026, 8, 1), "alice", True, False, None, True),
        ("doc2", "reddit", date(2026, 8, 1), "bob", True, False, None, True),
    ])
...
def test_the_document_booleans_are_read_and_not_a_sentinel():
    ...
    """
    conn = _Conn([
        ("doc1", "reddit", date(2026, 8, 1), "alice", True, True, None, True),
        ("doc2", "reddit", date(2026, 8, 1), "bob", False, False, None, True),
    ])
    facts, _ = _document_facts(conn, {"doc1", "doc2"})
...
def test_a_null_column_arrives_as_none_and_is_not_coerced():
    ...
    """
    conn = _Conn([("doc1", "reddit", date(2026, 8, 1), "alice", None, None, None, True)])
    facts, _ = _document_facts(conn, {"doc1"})
